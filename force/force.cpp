...
void *getFTData(void *arg)
{
    ...
    *(uint32*)&request[4] = htonl(NUM_SAMPLES); /* see section 9.1 in Net F/T user manual. */
    ...
    he = gethostbyname("192.38.66.240");
	...
	usleep(830); // ~ 1200 Hz --> run a bit faster than FT broadcast frequency -- currently 1000 Hz
    }
}
...
void forceControl(UrDriver *ur5, std::condition_variable *rt_msg_cond_, int run_time, int rotation_on, double f_ref)
{
    ...
    std::cout << "Force control initiated - starting circular motion ...\n";
	...
	ur5->setSpeed(speed[0], speed[1], speed[2], speed[3], speed[4], speed[5], 10);
	...
	forcelog << elapsTime << " " 
		 ...
		 << u_Fx << " " << u_Fy << " " << u_Fz << " "
		 << current_TCP_Frame[0] << " " << current_TCP_Frame[1] << " " << current_TCP_Frame[2] << " "
		 << ref_TCP_Frame[0] << " " << ref_TCP_Frame[1] << " " << ref_TCP_Frame[2] << " "
		 << current_TCP_Frame_adjusted[0] << " " << current_TCP_Frame_adjusted[1] << " "
		 << new_TCP_Frame[0] << " " << new_TCP_Frame[1] << " "
