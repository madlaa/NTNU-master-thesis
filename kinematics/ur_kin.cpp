...
namespace ur_kinematics {
  ...
  void forward(const double* q, double T06[16]) {
...
int getnearest(double T06[16], const double* q, double* qout) {
  double T1[16];
  T1[0] = -1;	T1[1] = 0;   T1[2] = 0;	 T1[3] = 0;
  T1[4] = 0;	T1[5] =	-1;  T1[6] = 0;	 T1[7] = 0;
  T1[8] = 0;	T1[9] =	0;   T1[10] = 1; T1[11] = 0;
  ...
  double T2[16];
  T2[0] = 0;	T2[1] = -1; T2[2] = 0;	T2[3] = 0;
  T2[4] = 0;	T2[5] =	0;  T2[6] = -1;	T2[7] = 0;
  T2[8] = 1;	T2[9] =	0;  T2[10] = 0; T2[11] = 0;
  T2[12] = 0;	T2[13] = 0; T2[14] = 0; T2[15] = 1;
  ...
  double inv_kin_sol[8*6], smallestAngleSum; // Getting max of 8 inverse kinematic solutions.
  int numsol, i, j, smallest = -1, angleSum;
  numsol = ur_kinematics::inverse(H, &inv_kin_sol[0]);
  if (numsol == 0) {
    return 1;
  }
  for (i = 0; i<numsol; i++) {
    angleSum = 0;
    for (j = 0; j < 6; j++) {
      angleSum += pow(shortestdist(inv_kin_sol[i*6+j],q[j]),2);
      //std::cout << inv_kin_sol[i*6+j] << " ";
      }
    //std::cout << "\n";
    if (smallest < 0) {
      smallest = i;
    ...
    else if (angleSum < smallestAngleSum) {
      smallest = i;
      smallestAngleSum = angleSum;
    }
  }
  for (i=0; i<6; i++) {
    qout[i] = putnear(inv_kin_sol[smallest*6+i],q[i]);
  }
  return 0;
}

double shortestdist(double angle1, double angle2) {
  double bigger = (angle1 >= angle2) ? angle1 : angle2;
  double smaller = (bigger == angle1) ? angle2 : angle1;
  while ((bigger - smaller) > 2 * ur_kinematics::PI) bigger -= 2*M_PI;
  double result = bigger-smaller;
  if (result > M_PI) result = 2*M_PI - result;
  return result;
}

double putnear(double original, double target) {
  int sign = sgn(original - target);
  while (fabs(original-target) > fabs((original - sign*2* ur_kinematics::PI)-target)) original -= sign*2* ur_kinematics::PI;
  return original;
}
